...
public class MainWrapper {
  ...
  // Person에서 public Person(){}, get, set 사용
  public static void ex01() {
    ...
    // 디폴트 생성자 + Setter
    for(int i = 0; i < f.getFamily().length; i++) {    // family.getFamily().length = familyCount
  ...
  // Person에서 name, age를 파라미터로 가진 메소드 사용
  ...
  public static void ex04() {
    ...
    family[1] = null;
    for(Person p : family) {
      if(p != null) {           //family[1]의 null값 때문에 오류가 나기 때문에 이와 같이 예외처리를 해주어야 함
        System.out.println("가족 이름 : " + p.getName());
        System.out.println("가족 나이 : " + p.getAge());
      }
    }
