...
//    absNumber = number < 0 ? -1 * number : number;
  ...
  public static void q7() {
	...
	public static void q8() {
		...
		String answer = number / 1000000 % 2 == 1 ? "남자" : "여자";
	...
	// 앞 뒤로 숫자를 분리 -> 각 숫자를 더함 -> 더한 수를 제곱을 구함 -> 자신과 같은가?
	// 81 -> 8   1로 분리 -> 9 -> 9 * 9 -> 81
	public static void q8_1() {
	  int n = 81;
	  int a = 0;
	  if(n >= 10 && n <= 99) {
	    a = (n / 10 + n % 10) * (n / 10 + n % 10);
	  } else if(n >= 1000 && n <= 9999) {
	    a = (n / 1000 + n % 1000) * (n / 1000 + n % 1000);	    
	  } else {
	    System.out.println("2자리(10 ~ 99) 또는 4자리(1000 ~ 9999) 수를 입력해주세요.");
	    return;
	  }
	  if(n == a) {
	    System.out.println("카프리카!");
	  }
	  else {
	    System.out.println("카프리카 아님!");
	  }
	}
	...
	public static void q9() {
	  ...
	  for(int i = 2; i < 6; i++) {
	    for(int j = 1; j < 10; j++) {
	      System.out.println(i + " x " + j + " = " + String.format("%4d", i * j));
	      if(i == 5 && j == 5){
	        break;
	      }
	    }
	  ...
	  */
	  // 위 코드는 안쪽for문만 끝나고 i++로 감
	  // 바깥쪽 for문을 끝내고 싶다면? 라벨을 붙인다!
	  outer:
	    for(int i = 2; i < 10; i++) {
	      for(int j = 1; j < 10; j++) {
	  System.out.println(i + " x " + j + " = " + String.format("%4d", i * j)); 
	...
	public static void q10() {
		...
		System.out.println(begin + "부터 " + end + "사이 모든 정수의 평균은 " + (double)total/count + "입니다.");
	...
	public static void q13() {
		...
		System.out.println("배열에 포함된 " + TARGET + "은(는) " + count + "개입니다.");
	...
	public static void q14() {
		...
		//String gift = null;   // 문자열 연결 시 null 사용 x -> null도 문자열 취급
/*		if(score >= 60) {
		  gift += "행주";
		}
		if(score >= 70) {
