 ...
 * 2. default(적지않음) : 동일한 패키지에서만 접근할 수 있다. (access modifier를 작성하지 않는 경우)
 ...
 * 1. Getter
 *    1) 필드값을 외부로 반환(reture)하는 메소드
 *    2) "get + 필드명" 형식의 이름을 가지는 메소드 ex) getId, getAge (카멜케이스 규칙)
 *    3) 메소드 이름을 마음대로 바꾸면 안 된다.
 * 2. Setter
 *    1) 외부로부터 받아온 값(파라미터)을 필드에 저장하는 메소드
 *    2) "set + 필드명" 형식의 이름을 가지는 메소드이다.
 *    3) 메소드 이름을 마음대로 바꾸면 안 된다.
 *    4) 반환타입이 없음(public void ...)
...
public class User {
  ...
  // setter : 외부로부터 받아온 값(파라미터)을 필드에 저장하는 메소드
