...
public class Library {
  ...
  private void addBook() throws InputMismatchException {
    ...
    String title = sc.next();   // 책 제목에 공백 포함X
  ...
  private void deleteBook() throws RuntimeException {
    ...
    for(Book book : books) {
      if(title.equals(book.getTitle())) {
        books.remove(book);   // 인덱스 기반이 아닌, 객체 기반으로 삭제!
  ...
  /**
   * 할인<br>
   * 할인을 적용할 책의 제목을 입력받아서 해당 책에 할인을 적용하는 메소드
   * @throws RuntimeException 책이 0권이거나 할인을 적용할 책의 제목이 입력되지 않았을 때 발생하는 예외
   * @throws InputMismatchException 할인을 적용할 퍼센트를 정수로 입력하지 않았을 때 발생하는 예외
   */
  
  public void saleBook() throws RuntimeException, InputMismatchException {
    System.out.println("================ 할인 ================");
    if(books.isEmpty()) {
      throw new RuntimeException("도서관에 등록된 책이 없습니다.");
    }
    System.out.println("할인을 적용할 책의 제목 입력 >>> ");
    String title = sc.next();
    if(title.isEmpty()) {
      throw new RuntimeException("할인을 적용할 책의 제목이 입력되지 않았습니다.");
    }
    for(Book book : books) {
      if(title.equals(book.getTitle())) {
        System.out.println("할인을 적용할 퍼센트(%) 입력 >>> ");
        int sale = sc.nextInt();
        book.setPrice((int)((1 - sale * 0.01) * book.getPrice()));
        System.out.println(title + " 책의 가격이 " + sale + "% 할인 적용되어 " + book.getPrice() + "원입니다.");
        return;
      }
    }
    System.out.println(title + " 제목을 가진 책이 없습니다.");
  ...
  public void manage() {
    ...
    while(true) {
      try {
        ...
        System.out.println("1.등록 2.삭제 3.수정 4.조회 5.전체조회 6.할인 0.종료");
