...
public class Ex05_Calendar {
  ...
  public static void ex01() {
    ...
    // 현재 날짜와 시간
    Calendar calendar = Calendar.getInstance();   // 객체 가져오기 만들어둔 객체를 가져다 쓰기만 함(new를 허용하지않음)
    
    // 구성요소 가져오기
    int year = calendar.get(Calendar.YEAR);          // Calendar.YEAR == 1 이므로 int year = calaendar.get(1)
    ...
    int hour24 = calendar.get(Calendar.HOUR_OF_DAY); // 24시각(00~23)
