...
public class MainWrapper {
  ...
  public static void ex01() {
    ...
    try {
      ...
      e.printStackTrace();
    } finally {   // 입력스트림은 사실 꼭! 닫지는 않아도 된다. 출력스트림은 꼭 !! 닫아야한다.
      try {
        if(fin != null) {
          fin.close();
        }
...
//      sb.append(new String(b, 0, readByte));
  ...
  public static void ex03() {
    ...
    /* 
     * java.io.BufferedInputStream - 가장 많이 사용!
...
//      sb.append(new String(b, 0, readByte));
  ...
  public static void ex05() {
     ...
     * 2. 직렬화(Serializable)된 객체를 입력받을 수 있다.
     * 3. 보조스트림이므로 메인스트림과 함께 사용한다.
     */
 ...
 // 디렉터리를 File 객체로 만들기
    ...
    try {
      ...
      // 객체 순서대로 입력 받기
      Student s = (Student)oin.readObject();    // 다운캐스팅
      
      // 결과 확인
      System.out.println(s);
      
    } catch(IOException | ClassNotFoundException e) {   // catch(Exception e)로도 가능
      e.printStackTrace();
    } finally {
      try {
        if(oin != null) {
